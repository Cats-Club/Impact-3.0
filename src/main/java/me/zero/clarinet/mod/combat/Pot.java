...
public class Pot extends Mod {
	...
	public Pot() {
		super("Pot", "Swaps to a potion in your hotbar and throws it", Keyboard.KEY_NONE, Category.COMBAT, false);
	}
	...
	public void onTick(EventTick event) {
		int pot = getPotFromHotbar();
		if (pot != -1) {
			switch (stage) {
				case 0:
					prevItem = mc.player.inventory.currentItem;
					mc.player.inventory.currentItem = pot;
					timer.reset();
					stage++;
					break;
				case 1:
	...
	private int getPotFromHotbar() {
		for (int i = 0; i < 9; i++) {
			...
			if (stack != null && stack.getItem() != null) {
				...
				if (hand instanceof ItemPotion) {
					ItemPotion pot = (ItemPotion) hand;
					if (PotionUtils.getEffectsFromStack(stack) != null) {
						for (PotionEffect effect : PotionUtils.getEffectsFromStack(stack)) {
							if (Potion.getIdFromPotion(effect.getPotion()) == 6 && pot instanceof ItemSplashPotion) {
								return i;
							}
						}
					}
