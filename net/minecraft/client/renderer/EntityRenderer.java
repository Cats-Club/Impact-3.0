...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        // TODON'T: Impact
        if (!flag) EventManager.call(new EventRender3D(partialTicks));
    ...
    public void updateFogColor(float partialTicks)
    {
        ...
        // TODON'T: Impact
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(MobEffects.BLINDNESS) && (!Impact.getInstance().getModManager().get(AntiBlind.class).isToggled()))
        {
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        // TODON'T: Impact
        else if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(MobEffects.BLINDNESS) && (!Impact.getInstance().getModManager().get(AntiBlind.class).isToggled()))
        {
            float f2 = 5.0F;
            int i = ((EntityLivingBase)entity).getActivePotionEffect(MobEffects.BLINDNESS).getDuration();
        ...
        // TODON'T: Impact
        else if (iblockstate.getMaterial() == Material.WATER && !(Impact.getInstance().getModManager().get(LiquidVision.class).isToggled() && Impact.getInstance().getModManager().get(LiquidVision.class).water.getValue()))
        {
            if (Config.isShaders())
            {
                Shaders.setFog(GlStateManager.FogMode.EXP);
        ...
        // TODON'T: Impact
        else if (iblockstate.getMaterial() == Material.LAVA && !(Impact.getInstance().getModManager().get(LiquidVision.class).isToggled() && Impact.getInstance().getModManager().get(LiquidVision.class).lava.getValue()))
        {
            if (Config.isShaders())
            {
                Shaders.setFog(GlStateManager.FogMode.EXP);
