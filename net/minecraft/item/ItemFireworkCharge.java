...
{
    public static NBTBase getExplosionTag(net.minecraft.item.ItemStack stack, String key)
    {
    ...
    public static void addExplosionInfo(NBTTagCompound nbt, List<String> tooltip)
    {
        ...
        if (aint.length > 0)
        {
            ...
            for (int i : aint)
            {
                ...
                for (int j = 0; j < net.minecraft.item.ItemDye.DYE_COLORS.length; ++j)
                {
                    if (i == net.minecraft.item.ItemDye.DYE_COLORS[j])
                    {
                        flag1 = true;
                        s = s + I18n.translateToLocal("item.fireworksCharge." + net.minecraft.item.EnumDyeColor.byDyeDamage(j).getUnlocalizedName());
                        break;
                    }
                }
