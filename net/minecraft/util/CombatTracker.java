...
{
    private final List<net.minecraft.util.CombatEntry> combatEntries = Lists.<net.minecraft.util.CombatEntry>newArrayList();
     ...
     */
    public void trackDamage(net.minecraft.util.DamageSource damageSrc, float healthIn, float damageAmount)
    {
        this.reset();
        this.calculateFallSuffix();
        net.minecraft.util.CombatEntry combatentry = new net.minecraft.util.CombatEntry(damageSrc, this.fighter.ticksExisted, healthIn, damageAmount, this.fallSuffix, this.fighter.fallDistance);
    ...
    public ITextComponent getDeathMessage()
    {
        ...
        {
            net.minecraft.util.CombatEntry combatentry = this.getBestCombatEntry();
            net.minecraft.util.CombatEntry combatentry1 = (net.minecraft.util.CombatEntry)this.combatEntries.get(this.combatEntries.size() - 1);
            ...
            if (combatentry != null && combatentry1.getDamageSrc() == net.minecraft.util.DamageSource.fall)
            {
                ITextComponent itextcomponent2 = combatentry.getDamageSrcDisplayName();

                if (combatentry.getDamageSrc() != net.minecraft.util.DamageSource.fall && combatentry.getDamageSrc() != net.minecraft.util.DamageSource.outOfWorld)
                {
    ...
    public EntityLivingBase getBestAttacker()
    {
        ...
        for (net.minecraft.util.CombatEntry combatentry : this.combatEntries)
        {
    ...
    @Nullable
    private net.minecraft.util.CombatEntry getBestCombatEntry()
    {
        net.minecraft.util.CombatEntry combatentry = null;
        net.minecraft.util.CombatEntry combatentry1 = null;
        float f = 0.0F;
        float f1 = 0.0F;

        for (int i = 0; i < this.combatEntries.size(); ++i)
        {
            net.minecraft.util.CombatEntry combatentry2 = (net.minecraft.util.CombatEntry)this.combatEntries.get(i);
            net.minecraft.util.CombatEntry combatentry3 = i > 0 ? (net.minecraft.util.CombatEntry)this.combatEntries.get(i - 1) : null;

            if ((combatentry2.getDamageSrc() == net.minecraft.util.DamageSource.fall || combatentry2.getDamageSrc() == DamageSource.outOfWorld) && combatentry2.getDamageAmount() > 0.0F && (combatentry == null || combatentry2.getDamageAmount() > f1))
            {
                if (i > 0)
                {
